...
const emptyForm = { description: "", amount: "", category_id: "", expense_date: todayISO() };
...
export default function ExpensesAdmin() {
    ...
    function openAdd() {
        setEditingId(null);
        setForm({ ...emptyForm, category_id: expenseCategories[0]?.id ?? "" });
        setModalOpen(true);
    }
    ...
    async function submitForm(event) {
        ...
        setError("");
        try {
            await apiFetch(editingId ? `/api/expenses/${editingId}` : "/api/expenses", {
    ...
    return (
        <div>
            <div className={shared.pageHeader}>
                <div>
                    <h1>Expenses</h1>
                    <p>{expenses.length} {expenses.length === 1 ? "entry" : "entries"} matching your filters</p>
                </div>
                <button type="button" className={shared.primaryButton} onClick={openAdd} disabled={!expenseCategories.length}>
                    + Add expense
                </button>
            </div>
                            ...
                            <tbody>
                                {expenses.map((expense) => (
                                    <tr key={expense.id}>
                                        ...
                                        <td><CategoryBadge name={expense.category} color={expense.category_color} /></td>
                                        <td>{formatCurrency(expense.amount, currency)}</td>
                                        <td>
                                            <div className={shared.actions}>
                                                <button type="button" className={shared.linkButton} onClick={() => openEdit(expense)}>Edit</button>
                                                <button type="button" className={shared.dangerLink} onClick={() => deleteExpense(expense.id)}>Delete</button>
                                            </div>
                                        </td>
                                    </tr>
                                ))}
                ...
                <Modal title={editingId ? "Edit expense" : "Add expense"} onClose={() => setModalOpen(false)}>
                    <form className={shared.form} onSubmit={submitForm}>
                        <div>
                            ...
                            <input id="description" name="description" value={form.description} onChange={updateField} placeholder="Morning coffee" required />
                        </div>
                        <div className={shared.formRow}>
                            ...
                            <div>
                                ...
                                <input id="expense_date" name="expense_date" type="date" value={form.expense_date} onChange={updateField} required />
                            </div>
                        </div>
                        <div>
